...
def points_to_gcode(points):
    ...
    for p in points[1:]:
        g.append(f"G1 X{p.x:.3f} Y{p.y:.3f} F{FEED}")
...
def line_to_gcode(e):
    ...
    return [
        ...
        pen_down(),
        f"G1 X{fin.x:.3f} Y{fin.y:.3f} F{FEED}",
        pen_up(),
    ]
...
def entity_to_gcode(e):
    match e.dxftype():
        ...
        case "INSERT":
            if e.dxf.name == "Borders Default Border" and borderOn == False:
                return []
            elif e.dxf.name[0:12] == "Title Blocks ANSI" and titleBlocksOn == False:
                return []
